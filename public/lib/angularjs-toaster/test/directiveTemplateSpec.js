...
describe('directiveTemplate', function () {
	...
	it('should load and render the referenced directives template text', function () {
		var container = compileContainer();
		pop({ type: 'info', body: 'bind-template-only', bodyOutputType: 'directive' });

		expect(container[0].innerText).toBe('×here is some great new text! It was brought in via directives!');
	});

	it('should bind directiveData to the directives template', function () {
		var container = compileContainer();
	...
	it('should throw an error if directiveName argument is not passed via body', function () {
		...
		try {
			...
			expect(e.message).toBe('A valid directives name must be provided via the toast body argument when using bodyOutputType: directive');
			hasError = true;
		}
	...
	it('should throw an error if the directives could not be found', function () {
		var hasError = false;

		compileContainer();

		try {
			pop({ type: 'info', body: 'non-existent-directives', bodyOutputType: 'directive' });
		} catch (e) {
			expect(e.message).toBe('non-existent-directives could not be found.');
			hasError = true;
		}
	...
	function createDirectives() {
		angular.module('testApp', [])
			.directive('bindTemplateOnly', function () {
				return {
					restrict: 'A',
					template: 'here is some great new text! <span style="color:orange">It was brought in via directives!</span>'
				}
			})
